...
# BS EN ISO 7730 Performance Assessment Method - ESP-r implementation
...
# Parse command line.
while getopts ":hvf:p:t:s:d:r:j:c:i:P:U" opt; do
  case "$opt" in
    h) information=true;;
    v) verbose=true;;
...
## Check model image format from file extension.
#if ! [ "X$model_image" == "X" ]; then
## Check model image exists.
#  if ! [ -f "$model_image" ]; then
#    echo "Error: model image not found." >&2
#    exit 107
#  fi
#  ext="${model_image##*.}"
#  if [ "$ext" != "pdf" ] && [ "$ext" != "png" ] && [ "$ext" != "jpg" ] && [ "$ext" != "eps" ]; then
#    echo "Error: model image does not appear to be a .pdf, .png, .jpg or .eps file." >&2
#    exit 107
#  fi
#fi
...
"$common_dir/esp-query/esp-query.py" -o "$tmp_dir/query_results.txt" "$building" "model_name" "model_description" "number_zones" "CFD_domains" "zone_control" "zone_setpoints" "MRT_sensors" "MRT_sensor_names" "afn_network" "zone_names" "zone_floor_surfs"
...
# Allow model description to be empty.
model_description="$(awk -f "$common_dir/esp-query/processOutput_getModelDescription.awk" "$tmp_dir/query_results.txt")"
...
number_eval_locs="$number_MRT_sensors"
if ! $is_MRT; then
...
is_MRTwoCFD=false
if [ "$number_zones_with_MRTsensors" -gt "$number_zones_with_CFDandMRT" ]; then
  is_MRTwoCFD=true
fi
...
if $do_simulation; then
  ...
  rm -f "$sim_results" > /dev/null
  if $is_afn; then rm -f "$mf_results" > /dev/null; fi
  if $is_CFD; then rm -f "$cfd_results" > /dev/null; fi
  rm -f "$building_dir"/ACC-actions_*.rec > /dev/null
  rm -f "$building_dir"/cfd3dascii_* > /dev/null

  # Update simulation year in cfg file.
  sed -i -e 's/\*year *[0-9]*/*year '"$year"'/' "$building"
...
for i0 in "${array_zone_indices[@]}"; do
  ...
  if [ "${array_MRT_sensors[i0]}" -eq 0 ]; then
    ...
    # For zones without MRT sensors, we put a -1 in the floor surfaces array.
    # Logic below ensures that the floor PTD array inherits "not occupied" entries for such zones.
    array_num_floor_surfaces[i0]=-1
...
if [ "$number_zones_with_MRTsensors" -gt 1 ]; then
  if [ "$number_MRT_sensors" -gt 41 ]; then
    ...
    while [ "$num_sensors_left" -gt 41 ]; do
      ...
      num_cols_left=41
      s=""
      if [ "$num_sensor_from_previous" -gt 0 ]; then
...
PTD_floor="$(echo "$deviation_floor" | awk -v ncols="$s" -f "$script_dir/get_variableMulticolumnPercentTimeDiscomfort.awk")"
...
severity_floor="$(echo "$PTD_floor" | awk -v ncols="$s" -f "$script_dir/get_severityRating.awk")"
...
for is0 in "${array_sensor_indices[@]}"; do
  ...
  if [ "$iz0" -gt "$iz0prev" ]; then ((izM0++)); fi
  ...
  if [ "${array_CFD_domains[iz0]}" -eq 2 ]; then
    ...
    # sevs="$sevs ${array_severity_vertdT[isCaM0]} ${array_severity_draught[isCaM0]}"
    # ((isCaM0++))
  fi
...
#echo "${array_zone_severity[@]}"
#echo "${array_sensor_severity[@]}"
